...
NUM_CLASSES = 6
W_SPATIAL = 0.7  # blend weight: 70% spatial model, 30% transition rates
...
def _build_training_data(round_dir: Path, detail: dict):
    ...
    # Build per-seed empirical counts
    empirical = {}
    for seed_idx in range(detail["seeds_count"]):
        state = detail["initial_states"][seed_idx]
        grid = np.array(state["grid"])
        H, W = grid.shape
        cell_counts = np.zeros((H, W, NUM_CLASSES), dtype=float)
        cell_totals = np.zeros((H, W), dtype=float)
        obs_file = round_dir / "observations" / f"seed_{seed_idx}.json"
        if obs_file.exists():
            with open(obs_file) as f:
                obs = json.load(f)
            for o in obs:
                ...
                for dy in range(len(o["grid"])):
                    for dx in range(len(o["grid"][0])):
                        ...
                        if gy >= H or gx >= W:
                            continue
                        ac = GRID_TO_CLASS.get(int(o["grid"][dy][dx]), 0)
                        cell_counts[gy, gx, ac] += 1
                        cell_totals[gy, gx] += 1
        empirical[seed_idx] = (cell_counts, cell_totals)

    return np.array(X_rows), np.array(Y_rows), trans, empirical
...
def predict_round(round_dir: str | Path, w_spatial: float = W_SPATIAL) -> list[np.ndarray]:
    ...
    X, Y, trans, empirical = _build_training_data(round_dir, detail)
    ...
    for seed_idx in range(detail["seeds_count"]):
        ...
        # For cells with 4+ direct observations, blend in empirical counts
        cell_counts, cell_totals = empirical.get(seed_idx, (np.zeros((H, W, NUM_CLASSES)), np.zeros((H, W))))
        obs_used = 0
        for y in range(H):
            for x in range(W):
                n_obs = cell_totals[y, x]
                if n_obs >= 4:
                    alpha = 0.5
                    emp_dist = (cell_counts[y, x] + alpha) / (n_obs + alpha * NUM_CLASSES)
                    w_emp = (n_obs - 3) / (n_obs + 3.0)
                    blended[y, x] = w_emp * emp_dist + (1 - w_emp) * blended[y, x]
                    obs_used += 1
        ...
        logger.info("Seed %d: done (%d cells with empirical override)", seed_idx, obs_used)
